...
class CEMOptimizer(Optimizer):
    ...
    def setup(self, cost_function):
        ...
        def sample_truncated_normal(shape, mu, sigma, a, b):
            ...
            '''
            uniform = torch.rand(shape)# 1000, 60, 4
            normal = torch.distributions.normal.Normal(0, 1)
             
            alpha = (a - mu) / sigma# 60,4
            beta = (b - mu) / sigma# 60,4
 
            alpha_normal_cdf = normal.cdf(alpha)# 60,4
 
            p = alpha_normal_cdf + (normal.cdf(beta) - alpha_normal_cdf) * uniform# 1000,60,4
            ...
            x = torch.clamp(x, a[0][0], b[0][0])
            # print(f"a[][] = {a[0][0]}")
            # print(f"b[][] = {b[0][0]}")
            # print(f'sample_truncated_normal returned shize{x.shape}')
            return x
    ...
    def obtain_solution(self, init_mean, init_var, use_pytorch=True, debug=False):
        ...
        if use_pytorch:
            ...
            size = [self.popsize, self.sol_dim,4]
        else:
        ...
        while (t < self.max_iters) and np.max(var) > self.epsilon:
            # print(f"size of mean={mean }")
            # print(f"size of self.lb={self.lb }")
            # print(f"size ofself.ub={self.ub }")
            ...
            # print("MPC elite sample cost sort:")
