...
from metagym.quadrotor.utils import TEXTURE_PATH, TILE, FACES
from metagym.quadrotor.utils import sectorize, cube_vertices, geometry_hash, \
...
class RenderWindow(pyglet.window.Window):
    ...
    def _draw_label(self,action):
        x, y, z = self.position
        x -= self.x_offset
        y -= self.y_offset
        y = -y
        a0,a1,a2,a3 = action[0],action[1],action[2],action[3]
        self.label.text = f'x:{x:.1f}, y:{y:.1f}, z:{z:.1f} t0:{a0:.1f}, t1:{a1:.1f}, t2:{a2:.1f}, t3:{a3:.1f}'
        self.label.draw()
