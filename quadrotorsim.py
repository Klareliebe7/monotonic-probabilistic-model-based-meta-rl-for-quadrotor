...
class QuadrotorSim(object):
    ...
    def get_sensor(self):
        ...
        barometer = self.global_position 
    ...
    def define_approching_task(self, dt, nt, start_global_point,end_global_point):
        """
        define the route coordinate of approching task
        """
        print(f"Starting point:{start_global_point} ")
        print(f"Approching point:{end_global_point} ")
        current_state = self._save_state()
        waypoint_lst = []
        for i in range(nt):
            waypoint_lst.append(list((end_global_point-start_global_point)*i/nt))
